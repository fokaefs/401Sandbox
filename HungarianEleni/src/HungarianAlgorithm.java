...
import fileManipulations.HungarianInputReader;

public class HungarianAlgorithm {
	
	
	
	private static final double NEUTRAL_VALUE = -99999;
	
	private static int maxReviews;
	...
	public static int[][] hgAlgorithm (double[][] array, String sumType, HashMap<Integer,TreeSet<Integer>> finalAssignment)
	{
		...
		for (int i=0; i<mask.length; i++)
		{
			for (int j=0; j<mask[i].length; j++)
			{
				if (mask[i][j] == 1)
				{
					if(finalAssignment.containsKey(i)) {
						if(finalAssignment.get(i).size()<2*maxReviews) {
							assignment[i][0] = i;
							assignment[i][1] = j;
						}
					}
	...
	public static void main(String[] args) 
	{
		...
		/*try {
			BufferedReader in = new BufferedReader(new FileReader(new File("files/reviewers.txt")));
			...
			in.close();
			in = new BufferedReader(new FileReader(new File("files/people.txt")));
			line = in.readLine();
			while(line != null) {
				people.add(line);
				line = in.readLine();
		...
		//INPUT FROM FILE
		HungarianInputReader inputReader = new HungarianInputReader();
		
		String filename = "";
		String outputFile = "";
		if(args.length == 0) {
			filename = "files/Evaluator-PNI_Conflicts.csv";
			outputFile = "files/assignments.csv";
		}
		else {
			filename = args[0];
			outputFile = args[1];
		}
		double[][] array = inputReader.readInputFromFile(filename);
		ArrayList<String> reviewers = inputReader.getReviewers();
		ArrayList<String> people = inputReader.getPeople();
		
		maxReviews = inputReader.getMaxReviews();
		boolean transposed = false;
		if (array.length > array[0].length)
		{
			System.out.println("Array transposed (because rows="+array.length+">columns="+array[0].length+").\n");	//Cols must be >= Rows.
			array = transpose(array);
			transposed = true;
			//MAX_REVIEWS = (int) ceil((double)array.length/(double)array[0].length);
		}
		else {
			//MAX_REVIEWS = (int) ceil((double)array[0].length/(double)array.length);
		}
		double[][] originalArray = DoubleArray.copy(array);
		HashMap<Integer,TreeSet<Integer>> finalAssignment = new HashMap<Integer,TreeSet<Integer>>();
				
		//<COMMENT> TO AVOID PRINTING THE MATRIX FOR WHICH THE ASSIGNMENT IS CALCULATED
		System.out.println("\n(Printing out only 2 decimals)\n");
		System.out.println("The matrix at round 1 is:");
		for (int i=0; i<array.length; i++)
		{
			for (int j=0; j<array[i].length; j++)
		...
		int[][] assignment = runAssignmentRound(sumType, array, transposed, finalAssignment);
		double endTime = System.nanoTime();
		
		array = DoubleArray.copy(originalArray);
		for(Integer i : finalAssignment.keySet()) {
			for(Integer j : finalAssignment.get(i)) {
				array[i][j] = NEUTRAL_VALUE;
			}
		}
		System.out.println("\n(Printing out only 2 decimals)\n");
		System.out.println("The matrix at round 2 is:");
		for (int k=0; k<array.length; k++)
		{
			for (int j=0; j<array[k].length; j++)
				{System.out.printf("%.2f\t", array[k][j]);}
			System.out.println();
		}
		System.out.println();
		assignment = runAssignmentRound(sumType, array, transposed, finalAssignment);				
		System.out.println("The winning assignment is:\n");	
		double sum = 0;
		for (int i=0; i<assignment.length; i++)
		{
		...
		printFinalAssignment(outputFile, reviewers, people, originalArray,
				finalAssignment);
		for(Integer i : finalAssignment.keySet()) {
			System.out.print(reviewers.get(i)+"\t");
			for(Integer j : finalAssignment.get(i)) {
				System.out.print(people.get(j)+"("+originalArray[i][j]+")"+"\t");
			}
			System.out.println();
		}
		System.out.printf("\nThe %s is: %.2f\n", sumType, sum);
	...
	private static void printFinalAssignment(String outputFile,
			ArrayList<String> reviewers, ArrayList<String> people,
			double[][] originalArray,
			HashMap<Integer, TreeSet<Integer>> finalAssignment) {
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(new File(outputFile)));
			out.write(",");
			for(Integer i : finalAssignment.keySet()) {
				out.write(reviewers.get(i)+",");
			}
			out.newLine();
			for(int i=0; i<people.size(); i++) {
				out.write(people.get(i)+",");
				for(int j=0; j<reviewers.size(); j++) {
					if (finalAssignment.get(j).contains(i)) {
						out.write(originalArray[j][i] + ",");
					}
					else {
						out.write(-100+",");
					}
	...
	private static int[][] runAssignmentRound(String sumType, double[][] array,
			boolean transposed, HashMap<Integer,TreeSet<Integer>> finalAssignment) {
		int[][] assignment=null;
		//int i = 0;
		//while(!isNeutralized(array)) {
		for(int i=0; i<maxReviews; i++) {
			assignment = new int[array.length][2];
			assignment = hgAlgorithm(array, sumType, finalAssignment); //Call Hungarian algorithm.
			for(int j=0; j<assignment.length; j++) {
				TreeSet<Integer> iAssignments=null;
				if(finalAssignment.containsKey(assignment[j][0])) {
					iAssignments = finalAssignment.get(assignment[j][0]);
				}
				else {
					iAssignments = new TreeSet<Integer>();
					
					
				}
				iAssignments.add(assignment[j][1]);
				finalAssignment.put(assignment[j][0], iAssignments);
				array = removeAssigned(array, assignment[j][1], transposed);
			}
			System.out.println("\n(Printing out only 2 decimals)\n");
			System.out.println("The matrix at step "+i+" is:");
			for (int k=0; k<array.length; k++)
			{
				for (int j=0; j<array[k].length; j++)
					{System.out.printf("%.2f\t", array[k][j]);}
	...
	private static boolean isNeutralized(double[][] array) {
		for(int i=0; i<array.length; i++) {
			for(int j=0; j<array[0].length; j++) {
				if(array[i][j] != NEUTRAL_VALUE) {
					return false;
				}
			}
		}
	...
	private static double[][] removeAssigned(double[][] array, int i, boolean transposed) {
		if(transposed) {
			double[] zeros = DoubleArray.fill(array.length, NEUTRAL_VALUE);
			array = DoubleArray.deleteColumns(array, i);
			array = DoubleArray.insertColumns(array, i, zeros);
			
		}
		else {
			double[] zeros = DoubleArray.fill(array[0].length, NEUTRAL_VALUE);
			array = DoubleArray.deleteRows(array, i);
			array = DoubleArray.insertRows(array, i, zeros);
		}
		return array;
	}
}
